			...
			try {
				...
				mbOauth.startDialogAuth(context, new String[] { "basic",
					...
					public void onException(String arg0) {
						Toast.makeText(context, arg0, Toast.LENGTH_SHORT)
								.show();
					}
				});
			} catch (Exception e) {
		...
		if (null != PCSDemoInfo.access_token) {
			...
			Thread workThread = new Thread(new Runnable() {
				...
				public void run() {
					...
					Log.v("carlos", "list response list is "
							+ listResponse.list);
						...
						public void run() {
							...
							if (listResponse.list != null) {
								if (!listResponse.list.isEmpty()) {
									...
									// Clear content list
									Log.v("carlos",
											"try to show the folder is empty");
